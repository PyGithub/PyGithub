...
def _db_id_and_type_to_node_id(db_id: int, typename: str) -> str:
	return "0" + str(len(typename)) + ":" + typename + str(db_id)

def db_id_and_type_to_node_id(db_id: int, typename: str) -> str:
	"""Converts GitHub DB ID of an object into its GraphQL ID."""
	return b64encode(_db_id_and_type_to_node_id(db_id, typename).encode("ascii")).decode("ascii")

def _node_id_to_db_id_and_type(node_id: str) -> (int, str):
	type_len, rest = node_id.split(":")
	if not type_len or typeLen[0] != '0':
