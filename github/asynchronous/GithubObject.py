...
class _NotSetType(Attribute[Any]):
    ...
    def remove_unset_items(data: dict[str, Any]) -> dict[str, Any]:
        return {
            key: value for key, value in data.items() if not isinstance(value, (_NotSetType, _sync_gho._NotSetType))
        }
...
class GithubObject(ABC):
    ...
    @property
    async def raw_data(self) -> dict[str, Any]:
        """
        :type: dict
        """
    ...
    @property
    async def raw_headers(self) -> dict[str, str | int]:
        """
        :type: dict
        """
...
class CompletableGithubObject(GithubObject, ABC):
    ...
    async def _completeIfNotSet(self, value: Attribute) -> None:
    ...
    async def update(self, additional_headers: dict[str, Any] | None = None) -> bool:
        ...
        if status == 304:
            ...
            headers, data = await self._requester._Requester__check(status, responseHeaders, output)  # type: ignore
            self._storeAndUseAttributes(headers, data)
            self.__completed = True
            return True
