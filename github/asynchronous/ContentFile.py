...
class ContentFile(CompletableGithubObject):
    ...
    async def content(self) -> str:
        await self._completeIfNotSet(self._content)
        return self._content.value

    @property
    async def decoded_content(self) -> bytes:
        assert (await self.encoding) == "base64", f"unsupported encoding: {await self.encoding}"
        return base64.b64decode(bytearray(await self.content, "utf-8"))

    @property
    async def download_url(self) -> str:
        await self._completeIfNotSet(self._download_url)
        return self._download_url.value

    @property
    async def encoding(self) -> str:
        await self._completeIfNotSet(self._encoding)
        return self._encoding.value

    @property
    async def file_size(self) -> int:
        await self._completeIfNotSet(self._file_size)
    ...
    @property
    async def last_modified_at(self) -> datetime:
        await self._completeIfNotSet(self._last_modified_at)
    ...
    @property
    async def line_numbers(self) -> list[str]:
        await self._completeIfNotSet(self._line_numbers)
    ...
    @property
    async def submodule_git_url(self) -> str:
        await self._completeIfNotSet(self._submodule_git_url)
        return self._submodule_git_url.value

    @property
    async def target(self) -> str:
        await self._completeIfNotSet(self._target)
        return self._target.value

    @property
    async def text_matches(self) -> str:
