...
class Label(CompletableGithubObject):
    ...
    async def _identity(self) -> str:
        return urllib.parse.quote(await self.name)

    @property
    async def color(self) -> str:
        await self._completeIfNotSet(self._color)
        return self._color.value

    @property
    async def default(self) -> bool:
        await self._completeIfNotSet(self._default)
    ...
    async def edit(self, name: str, color: str, description: Opt[str] = NotSet) -> None:
        """
