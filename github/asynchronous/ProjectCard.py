...
class ProjectCard(NonCompletableGithubObject):
    ...
    @property
    def creator(self) -> NamedUser.NamedUser:
    ...
    def get_content(self, content_type: Opt[str] = NotSet) -> PullRequest.PullRequest | Issue.Issue | None:
        """
        ...
        """
        assert is_undefined(content_type) or isinstance(content_type, str), content_type
        if self.content_url is None:
            return None

        retclass: type[PullRequest.PullRequest] | type[Issue.Issue]
        if content_type == "PullRequest":
            url = self.content_url.replace("issues", "pulls")
            retclass = PullRequest.PullRequest
        elif is_undefined(content_type) or content_type == "Issue":
            url = self.content_url
            retclass = Issue.Issue
        else:
            raise ValueError(f"Unknown content type: {content_type}")
        return retclass(self._requester, url=url)

    async def move(self, position: str, column: ProjectColumn.ProjectColumn | int) -> bool:
        """
        ...
        assert isinstance(column, (ProjectColumn.ProjectColumn, github.ProjectColumn.ProjectColumn)) or isinstance(
            column, int
        ), column
        post_parameters = {
            "position": position,
    ...
    async def edit(self, note: Opt[str] = NotSet, archived: Opt[bool] = NotSet) -> None:
        """
        :calls: `PATCH /projects/columns/cards/{card_id} <https://docs.github.com/en/rest/reference/projects#cards>`_
        """
        assert is_undefined(note) or isinstance(note, str), note
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "creator" in attributes:  # pragma no branch
            self._creator = self._makeClassAttribute(NamedUser.NamedUser, attributes["creator"])
