...
class BypassPullRequestAllowances(NonCompletableGithubObject):
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        if "apps" in attributes:  # pragma no branch
            self._apps = self._makeListOfClassesAttribute(GithubApp.GithubApp, attributes["apps"])
        if "teams" in attributes:  # pragma no branch
            self._teams = self._makeListOfClassesAttribute(Team.Team, attributes["teams"])
        if "users" in attributes:  # pragma no branch
            self._users = self._makeListOfClassesAttribute(NamedUser.NamedUser, attributes["users"])
...
class RequiredPullRequestReviews(CompletableGithubObject):
    ...
    @property
    async def bypass_pull_request_allowances(self) -> BypassPullRequestAllowances:
        await self._completeIfNotSet(self._bypass_pull_request_allowances)
        return self._bypass_pull_request_allowances.value

    @property
    async def dismiss_stale_reviews(self) -> bool:
        await self._completeIfNotSet(self._dismiss_stale_reviews)
        return self._dismiss_stale_reviews.value

    @property
    async def dismissal_restrictions(self) -> DismissalRestrictions:
        await self._completeIfNotSet(self._dismissal_restrictions)
    ...
    async def require_code_owner_reviews(self) -> bool:
        await self._completeIfNotSet(self._require_code_owner_reviews)
        return self._require_code_owner_reviews.value

    @property
    async def require_last_push_approval(self) -> bool:
        await self._completeIfNotSet(self._require_last_push_approval)
        return self._require_last_push_approval.value

    @property
    async def required_approving_review_count(self) -> int:
        await self._completeIfNotSet(self._required_approving_review_count)
