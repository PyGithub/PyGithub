...
class GithubIntegration:
    ...
    def __init__(
        ...
        *,
        ...
        """
        :param integration_id: int deprecated, use auth=Auth.AppAuth(...) instead
        :param private_key: string deprecated, use auth=Auth.AppAuth(...) instead
        ...
        :param jwt_expiry: int deprecated, use auth=Auth.AppAuth(...) instead
        :param jwt_issued_at: int deprecated, use auth=Auth.AppAuth(...) instead
        :param jwt_algorithm: string deprecated, use auth=Auth.AppAuth(...) instead
        ...
        if (
            ...
            warnings.warn(
                ...
                "please use auth=Auth.AppAuth(...) instead",
        ...
        assert isinstance(auth, AppAuth), f"GithubIntegration requires Auth.AppAuth authentication, not {type(auth)}"
    ...
    async def _get_installed_app(self, url: str) -> Installation:
        """
        Get installation for the given URL.
        """
        headers, response = await self.__requester.requestJsonAndCheck("GET", url, headers=self._get_headers())
    ...
    @deprecated(
        "Use github.Github(auth=Auth.AppAuth), Auth.AppAuth.token or Auth.AppAuth.create_jwt(expiration) instead"
    )
    ...
    async def get_access_token(
        ...
        headers, response = await self.__requester.requestJsonAndCheck(
            "POST",
    ...
    async def get_installation(self, owner: str, repo: str) -> Installation:
        """
        ...
        return await self._get_installed_app(url=f"/repos/{owner}/{repo}/installation")
    ...
    async def get_org_installation(self, org: str) -> Installation:
        """
        ...
        return await self._get_installed_app(url=f"/orgs/{org}/installation")

    async def get_repo_installation(self, owner: str, repo: str) -> Installation:
        """
    ...
    async def get_user_installation(self, username: str) -> Installation:
        """
        ...
        return await self._get_installed_app(url=f"/users/{username}/installation")

    async def get_app_installation(self, installation_id: int) -> Installation:
        """
        ...
        """
        return await self._get_installed_app(url=f"/app/installations/{installation_id}")

    async def get_app(self) -> GithubApp:
        """
        :calls: `GET /app <https://docs.github.com/en/rest/reference/apps#get-the-authenticated-app>`_
        """

        headers, data = await self.__requester.requestJsonAndCheck("GET", "/app", headers=self._get_headers())
