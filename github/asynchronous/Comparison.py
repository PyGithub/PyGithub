...
class Comparison(CompletableGithubObject):
    ...
    def _initAttributes(self) -> None:
        ...
        self._merge_base_commit: Attribute[Commit.Commit] = NotSet
    ...
    @property
    async def ahead_by(self) -> int:
        await self._completeIfNotSet(self._ahead_by)
        return self._ahead_by.value

    @property
    async def base_commit(self) -> Commit.Commit:
        await self._completeIfNotSet(self._base_commit)
        return self._base_commit.value

    @property
    async def behind_by(self) -> int:
    ...
    @property
    async def merge_base_commit(self) -> Commit.Commit:
        await self._completeIfNotSet(self._merge_base_commit)
    ...
    @property
    async def permalink_url(self) -> str:
        await self._completeIfNotSet(self._permalink_url)
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "base_commit" in attributes:  # pragma no branch
            self._base_commit = self._makeClassAttribute(Commit.Commit, attributes["base_commit"])
        ...
        if "commits" in attributes:  # pragma no branch
            self._commits = self._makeListOfClassesAttribute(Commit.Commit, attributes["commits"])
        ...
        if "merge_base_commit" in attributes:  # pragma no branch
            self._merge_base_commit = self._makeClassAttribute(Commit.Commit, attributes["merge_base_commit"])
