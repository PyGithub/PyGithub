...
class CheckRun(CompletableGithubObject):
    ...
    @property
    async def completed_at(self) -> datetime | None:
    ...
    @property
    async def details_url(self) -> str:
        await self._completeIfNotSet(self._details_url)
        return self._details_url.value

    @property
    async def external_id(self) -> str:
        await self._completeIfNotSet(self._external_id)
    ...
    def get_annotations(self) -> PaginatedList[CheckRunAnnotation]:
        ...
        return PaginatedList(
            CheckRunAnnotation.CheckRunAnnotation,
            self._requester,
    ...
    async def edit(
        ...
        headers, data = await self._requester.requestJsonAndCheck("PATCH", self.url, input=post_parameters)
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "check_suite" in attributes and "id" in attributes["check_suite"]:  # pragma no branch
            ...
            self._check_suite = self._makeClassAttribute(CheckSuite.CheckSuite, attributes["check_suite"])
        ...
        if "deployment" in attributes:  # pragma no branch
            self._deployment = self._makeClassAttribute(Deployment.Deployment, attributes["deployment"])
        ...
        if "output" in attributes:  # pragma no branch
            self._output = self._makeClassAttribute(CheckRunOutput.CheckRunOutput, attributes["output"])
