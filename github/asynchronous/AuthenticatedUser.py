...
class AuthenticatedUser(CompletableGithubObject):
    ...
    async def bio(self) -> str:
        await self._completeIfNotSet(self._bio)
    ...
    @property
    async def business_plus(self) -> bool:
        await self._completeIfNotSet(self._business_plus)
    ...
    @property
    async def hireable(self) -> bool:
        await self._completeIfNotSet(self._hireable)
    ...
    @property
    async def notification_email(self) -> str:
        await self._completeIfNotSet(self._notification_email)
    ...
    @property
    async def two_factor_authentication(self) -> bool:
        await self._completeIfNotSet(self._two_factor_authentication)
    ...
    async def add_to_emails(self, *emails: str) -> None:
        """
    ...
    async def add_to_following(self, following: NamedUser) -> None:
        ...
        """
        assert isinstance(following, (NamedUser.NamedUser, github.NamedUser.NamedUser)), following
    ...
    async def add_to_subscriptions(self, subscription: Repository) -> None:
        ...
        """
        assert isinstance(subscription, (Repository.Repository, github.Repository.Repository)), subscription
        headers, data = await self._requester.requestJsonAndCheck(
    ...
    async def add_to_watched(self, watched: Repository) -> None:
        ...
        """
        assert isinstance(watched, (Repository.Repository, github.Repository.Repository)), watched
        headers, data = await self._requester.requestJsonAndCheck(
            "PUT",
            f"/repos/{(await watched._identity)}/subscription",
    ...
    async def create_repo_from_template(
        ...
        headers, data = await self._requester.requestJsonAndCheck(
            "POST",
            f"/repos/{(await repo.full_name)}/generate",
            input=post_parameters,
    ...
    async def create_repo(
        ...
        headers, data = await self._requester.requestJsonAndCheck("POST", "/user/repos", input=post_parameters)
        return Repository.Repository(self._requester, headers, data, completed=True)
    ...
    async def edit(
        ...
        headers, data = await self._requester.requestJsonAndCheck("PATCH", "/user", input=post_parameters)
    ...
    def get_authorizations(self) -> PaginatedList[Authorization]:
        ...
        """
        return PaginatedList(Authorization.Authorization, self._requester, "/authorizations", None)

    async def get_emails(self) -> list[EmailData]:
        """
    ...
    def get_gists(self, since: Opt[datetime] = NotSet) -> PaginatedList[Gist]:
        ...
        return PaginatedList(Gist.Gist, self._requester, "/gists", url_parameters)
    ...
    async def get_issues(
        ...
        if is_defined(since):
            url_parameters["since"] = since.strftime("%Y-%m-%dT%H:%M:%SZ")
        return PaginatedList(Issue.Issue, self._requester, "/issues", url_parameters)
    ...
    async def get_notification(self, id: str) -> Notification:
        ...
        headers, data = await self._requester.requestJsonAndCheck("GET", f"/notifications/threads/{id}")
        return Notification.Notification(self._requester, headers, data, completed=True)
    ...
    async def get_organization_events(self, org: Organization) -> PaginatedList[Event]:
        ...
        """
        assert isinstance(org, (Organization.Organization, github.Organization.Organization)), org
        return PaginatedList(
            Event.Event,
            self._requester,
            f"/users/{await self.login}/events/orgs/{(await org.login)}",
            None,
        )
    ...
    async def has_in_following(self, following: NamedUser) -> bool:
        ...
        status, headers, data = await self._requester.requestJson(
            "GET", f"/user/following/{(await following._identity)}"
        )
        return status == 204
    ...
    async def has_in_subscriptions(self, subscription: Repository) -> bool:
        ...
        assert isinstance(subscription, (Repository.Repository, github.Repository.Repository)), subscription
        status, headers, data = await self._requester.requestJson(
            "GET", f"/user/subscriptions/{(await subscription._identity)}"
        )
        return status == 204
    ...
    async def has_in_watched(self, watched: Repository) -> bool:
        ...
        assert isinstance(watched, (Repository.Repository, github.Repository.Repository)), watched
        status, headers, data = await self._requester.requestJson(
            "GET", f"/repos/{(await watched._identity)}/subscription"
        )
        return status == 200

    async def mark_notifications_as_read(self, last_read_at: datetime | None = None) -> None:
        """
        ...
        headers, data = await self._requester.requestJsonAndCheck("PUT", "/notifications", input=put_parameters)

    async def remove_from_emails(self, *emails: str) -> None:
        """
    ...
    async def accept_invitation(self, invitation: Invitation | int) -> None:
        ...
        """
        assert isinstance(invitation, (Invitation.Invitation, github.Invitation.Invitation)) or isinstance(
            invitation, int
        )
    ...
    async def get_organization_membership(self, org: str) -> Membership:
        ...
        headers, data = await self._requester.requestJsonAndCheck("GET", f"/user/memberships/orgs/{org}")
