...
class HTTPSRequestsConnectionClass:
    ...
    async def getresponse(self) -> RequestsResponse:
...
class Requester:
    ...
    async def close(self) -> None:
        ...
        while self.__custom_connections:
            await self.__custom_connections.popleft().close()
    ...
    async def requestJsonAndCheck(
        ...
        """
        return await self.__postProcess(
            verb,
            url,
            *await self.__check(
                *await self.requestJson(
                    verb,
                    url,
                    parameters,
                    headers,
    ...
    async def requestMultipartAndCheck(
        self,
        verb: str,
        url: str,
        parameters: dict[str, Any] | None = None,
        ...
        return await self.__postProcess(
            ...
            url,
            *await self.__check(
                *await self.requestMultipart(verb, url, parameters, headers, input, self.__customConnection(url))
            ),
        )

    async def requestBlobAndCheck(
        self,
        verb: str,
        url: str,
        ...
        return await self.__postProcess(
            ...
            url,
            *await self.__check(
                *await self.requestBlob(verb, url, parameters, headers, input, self.__customConnection(url))
            ),
        )
    ...
    async def graphql_query(self, query: str, variables: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        """
        ...
        response_headers, data = await self.requestJsonAndCheck("POST", self.graphql_url, input=input_)
        if "errors" in data:
    ...
    async def graphql_query_class(
        ...
        """
        headers, data = await self.graphql_query(query, variables)
    ...
    async def graphql_node(
        ...
        headers, data = await self.graphql_query(query, {"id": node_id})
    ...
    async def graphql_node_class(
        ...
        headers, data = await self.graphql_node(node_id, output_schema, node_type)
        return self.data_as_class(headers, data, ["data", "node"], klass)

    async def graphql_named_mutation(
        ...
        headers, data = await self.graphql_query(query, {"input": mutation_input})
        return headers, data.get("data", {}).get(mutation_name, {})

    async def graphql_named_mutation_class(
        ...
        """
        headers, data = await self.graphql_named_mutation(mutation_name, mutation_input, output_schema)
    ...
    async def getFile(
        ...
        """
        _, _, stream_chunk_iterator = await self.getStream(url, parameters, headers, cnx, chunk_size=chunk_size)
    ...
    async def getStream(
        ...
        status, responseHeaders, output = await self.__requestEncode(
    ...
    async def requestMultipart(
        ...
        status, responseHeaders, output = await self.__requestEncode(cnx, verb, url, parameters, headers, input, encode)
        if isinstance(output, str):
            return status, responseHeaders, output
        raise ValueError("requestMultipart() Expected a str, should never happen")

    async def requestBlob(
        self,
        verb: str,
        url: str,
        parameters: dict[str, str] | None = None,
    ...
    async def requestMemoryBlobAndCheck(
        self,
        verb: str,
        url: str,
        parameters: Any,
        ...
        if isinstance(output, str):
            return await self.__postProcess(verb, url, *await self.__check(status, responseHeaders, output))
        raise ValueError("requestMemoryBlobAndCheck() Expected a str, should never happen")

    async def __requestEncode(
        self,
        ...
        status, responseHeaders, output = await self.__requestRaw(
    ...
    async def __requestRaw(
        ...
        await self.__deferRequest(verb)

        try:
            original_cnx = cnx
            if cnx is None:
                cnx = await self.__createConnection()
            cnx.request(verb, url, input, requestHeaders, stream)
            response = await cnx.getresponse()
            ...
            if status == 202 and (
                ...
                await asyncio.sleep(Consts.PROCESSING_202_WAIT_TIME)
                return await self.__requestRaw(original_cnx, verb, url, requestHeaders, input, stream=stream)
            ...
            if status == 301 and "location" in responseHeaders:
                ...
                return await self.__requestRaw(original_cnx, verb, o.path, requestHeaders, input, stream=stream)
    ...
    async def __deferRequest(self, verb: str) -> None:
