...
class Artifact(NonCompletableGithubObject):
    ...
    async def delete(self) -> bool:
        ...
        """
        status, headers, data = await self._requester.requestBlob("DELETE", self.url)
        return status == 204
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "workflow_run" in attributes:  # pragma no branch
            self._workflow_run = self._makeClassAttribute(WorkflowRun.WorkflowRun, attributes["workflow_run"])
