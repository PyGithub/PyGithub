...
class IssueEvent(CompletableGithubObject):
    ...
    @property
    async def author_association(self) -> dict[str, Any]:
    ...
    @property
    async def commit_url(self) -> str:
        await self._completeIfNotSet(self._commit_url)
    ...
    @property
    async def dismissed_review(self) -> dict:
        await self._completeIfNotSet(self._dismissed_review)
    ...
    @property
    async def lock_reason(self) -> str:
        await self._completeIfNotSet(self._lock_reason)
    ...
    @property
    async def project_card(self) -> dict[str, Any]:
        await self._completeIfNotSet(self._project_card)
        return self._project_card.value

    @property
    async def rename(self) -> dict:
        await self._completeIfNotSet(self._rename)
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "issue" in attributes:  # pragma no branch
            self._issue = self._makeClassAttribute(Issue.Issue, attributes["issue"])
        if "label" in attributes:  # pragma no branch
            self._label = self._makeClassAttribute(Label.Label, attributes["label"])
        ...
        if "requested_team" in attributes:  # pragma no branch
            self._requested_team = self._makeClassAttribute(Team.Team, attributes["requested_team"])
