...
class ProjectColumn(NonCompletableGithubObject):
    ...
    def get_cards(self, archived_state: Opt[str] = NotSet) -> PaginatedList[ProjectCard.ProjectCard]:
        """
        :calls: `GET /projects/columns/{column_id}/cards <https://docs.github.com/en/rest/reference/projects#list-project-cards>`_
        """
        assert is_undefined(archived_state) or isinstance(archived_state, str), archived_state

        url_parameters = dict()
        if is_defined(archived_state):
    ...
    async def create_card(
        ...
        if isinstance(note, str):
            assert is_undefined(content_id), content_id
            assert is_undefined(content_type), content_type
            post_parameters: dict[str, Any] = {"note": note}
        else:
            assert is_undefined(note), note
        ...
        return ProjectCard.ProjectCard(self._requester, headers, data)

    async def move(self, position: str) -> bool:
        """
