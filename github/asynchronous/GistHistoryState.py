...
class GistHistoryState(CompletableGithubObject):
    ...
    def _initAttributes(self) -> None:
        self._change_status: Attribute[CommitStats.CommitStats] = NotSet
        ...
        self._forks: Attribute[list[Gist.Gist]] = NotSet
    ...
    @property
    async def change_status(self) -> CommitStats.CommitStats:
        await self._completeIfNotSet(self._change_status)
    ...
    @property
    async def committed_at(self) -> datetime:
        await self._completeIfNotSet(self._committed_at)
    ...
    @property
    async def files(self) -> dict[str, GistFile]:
        await self._completeIfNotSet(self._files)
        return self._files.value

    @property
    async def forks(self) -> list[Gist.Gist]:
    ...
    async def version(self) -> str:
        await self._completeIfNotSet(self._version)
        return self._version.value

    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        if "change_status" in attributes:  # pragma no branch
            self._change_status = self._makeClassAttribute(CommitStats.CommitStats, attributes["change_status"])
        ...
        if "files" in attributes:  # pragma no branch
            self._files = self._makeDictOfStringsToClassesAttribute(GistFile, attributes["files"])
        if "forks" in attributes:  # pragma no branch
            self._forks = self._makeListOfClassesAttribute(Gist.Gist, attributes["forks"])
