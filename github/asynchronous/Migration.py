...
from . import GithubObject, NamedUser, Organization, PaginatedList, Repository
...
class Migration(CompletableGithubObject):
    ...
    async def exclude(self) -> list[str]:
        await self._completeIfNotSet(self._exclude)
        return self._exclude.value

    @property
    async def exclude_attachments(self) -> bool:
        await self._completeIfNotSet(self._exclude_attachments)
        return self._exclude_attachments.value

    @property
    async def exclude_git_data(self) -> bool:
        await self._completeIfNotSet(self._exclude_git_data)
        return self._exclude_git_data.value

    @property
    async def exclude_metadata(self) -> bool:
        await self._completeIfNotSet(self._exclude_metadata)
        return self._exclude_metadata.value

    @property
    async def exclude_owner_projects(self) -> bool:
        await self._completeIfNotSet(self._exclude_owner_projects)
        return self._exclude_owner_projects.value

    @property
    async def exclude_releases(self) -> bool:
        await self._completeIfNotSet(self._exclude_releases)
        return self._exclude_releases.value

    @property
    async def guid(self) -> str:
        await self._completeIfNotSet(self._guid)
    ...
    @property
    async def lock_repositories(self) -> bool:
        await self._completeIfNotSet(self._repositories)
    ...
    @property
    async def org_metadata_only(self) -> bool:
        await self._completeIfNotSet(self._org_metadata_only)
    ...
    @property
    async def repositories(self) -> list[Repository.Repository]:
        await self._completeIfNotSet(self._repositories)
    ...
    async def get_status(self) -> str:
        ...
        headers, data = await self._requester.requestJsonAndCheck(
            "GET", await self.url, headers={"Accept": Consts.mediaTypeMigrationPreview}
        )
        self._useAttributes(data)
        return await self.state

    async def get_archive_url(self) -> str:
        """
        :calls: `GET /user/migrations/{migration_id}/archive <https://docs.github.com/en/rest/reference/migrations>`_
        """
        headers, data = await self._requester.requestJsonAndCheck(
            "GET",
            f"{await self.url}/archive",
    ...
    async def unlock_repo(self, repo_name: str) -> None:
        """
        ...
        headers, data = await self._requester.requestJsonAndCheck(
            "DELETE",
            f"{await self.url}/repos/{repo_name}/lock",
            headers={"Accept": Consts.mediaTypeMigrationPreview},
        )
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "repositories" in attributes:
            self._repositories = self._makeListOfClassesAttribute(Repository.Repository, attributes["repositories"])
