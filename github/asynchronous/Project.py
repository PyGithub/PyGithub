...
from . import GithubObject, NamedUser, Organization, ProjectColumn
...
class Project(CompletableGithubObject):
    ...
    @property
    async def columns_url(self) -> str:
        await self._completeIfNotSet(self._columns_url)
    ...
    @property
    async def organization_permission(self) -> str:
        await self._completeIfNotSet(self._organization_permission)
        return self._organization_permission.value

    @property
    async def owner_url(self) -> str:
        await self._completeIfNotSet(self._owner_url)
    ...
    async def delete(self) -> None:
        ...
        headers, data = await self._requester.requestJsonAndCheck(
            "DELETE", await self.url, headers={"Accept": Consts.mediaTypeProjectsPreview}
        )
    ...
    async def edit(
        ...
        assert is_undefined(organization_permission) or isinstance(
    ...
    async def get_columns(self) -> PaginatedList[ProjectColumn.ProjectColumn]:
        """
        ...
        return PaginatedList(
            ...
            self._requester,
            await self.columns_url,
            None,
            headers={"Accept": Consts.mediaTypeProjectsPreview},
        )

    async def create_column(self, name: str) -> ProjectColumn.ProjectColumn:
        """
        ...
        headers, data = await self._requester.requestJsonAndCheck(
            "POST", f"{await self.url}/columns", headers=import_header, input=post_parameters
        )
        return ProjectColumn.ProjectColumn(self._requester, headers, data)
