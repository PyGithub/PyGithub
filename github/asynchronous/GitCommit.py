...
class GitCommit(CompletableGithubObject):
    ...
    async def comment_count(self) -> int:
        await self._completeIfNotSet(self._comment_count)
    ...
    @property
    async def parents(self) -> list[GitCommit]:
    ...
    async def timestamp(self) -> datetime:
        await self._completeIfNotSet(self._timestamp)
    ...
    @property
    async def tree_id(self) -> str:
        await self._completeIfNotSet(self._tree_id)
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        if "author" in attributes:  # pragma no branch
            self._author = self._makeClassAttribute(GitAuthor.GitAuthor, attributes["author"])
        ...
        if "committer" in attributes:  # pragma no branch
            self._committer = self._makeClassAttribute(GitAuthor.GitAuthor, attributes["committer"])
        ...
        if "tree" in attributes:  # pragma no branch
            self._tree = self._makeClassAttribute(GitTree.GitTree, attributes["tree"])
        ...
        if "verification" in attributes:  # pragma no branch
            self._verification = self._makeClassAttribute(
                GitCommitVerification.GitCommitVerification, attributes["verification"]
            )
