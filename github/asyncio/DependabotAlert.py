...
class DependabotAlert(NonCompletableGithubObject):
    ...
    def __repr__(self) -> str:
        return self.get__repr__({"number": self._number.value, "ghsa_id": self.security_advisory.ghsa_id})
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "dependency" in attributes:
            self._dependency = self._makeClassAttribute(
                DependabotAlertDependency.DependabotAlertDependency, attributes["dependency"]
            )
        ...
        if "security_advisory" in attributes:
            self._security_advisory = self._makeClassAttribute(
                DependabotAlertAdvisory.DependabotAlertAdvisory, attributes["security_advisory"]
            )
        if "security_vulnerability" in attributes:
            self._security_vulnerability = self._makeClassAttribute(
                DependabotAlertVulnerability.DependabotAlertVulnerability, attributes["security_vulnerability"]
            )
