...
class SourceImport(CompletableGithubObject):
    ...
    @property
    async def authors_count(self) -> int:
        await self._completeIfNotSet(self._authors_count)
        return self._authors_count.value

    @property
    async def authors_url(self) -> str:
        await self._completeIfNotSet(self._authors_url)
        return self._authors_url.value

    @property
    async def commit_count(self) -> int:
        await self._completeIfNotSet(self._commit_count)
        return self._commit_count.value

    @property
    async def error_message(self) -> str:
        await self._completeIfNotSet(self._error_message)
        return self._error_message.value

    @property
    async def failed_step(self) -> str:
        await self._completeIfNotSet(self._failed_step)
        return self._failed_step.value

    @property
    async def has_large_files(self) -> bool:
        await self._completeIfNotSet(self._has_large_files)
    ...
    @property
    async def import_percent(self) -> int:
        await self._completeIfNotSet(self._import_percent)
        return self._import_percent.value

    @property
    async def large_files_count(self) -> int:
        await self._completeIfNotSet(self._large_files_count)
        return self._large_files_count.value

    @property
    async def large_files_size(self) -> int:
        await self._completeIfNotSet(self._large_files_size)
    ...
    @property
    async def project_choices(self) -> list[dict[str, Any]]:
        await self._completeIfNotSet(self._project_choices)
        return self._project_choices.value

    @property
    async def push_percent(self) -> int:
        await self._completeIfNotSet(self._push_percent)
    ...
    async def status_text(self) -> str:
        await self._completeIfNotSet(self._status_text)
        return self._status_text.value

    @property
    async def svc_root(self) -> str:
        await self._completeIfNotSet(self._svc_root)
        return self._svc_root.value

    @property
    async def svn_root(self) -> str:
        await self._completeIfNotSet(self._svn_root)
        return self._svn_root.value

    @property
    async def tfvc_project(self) -> str:
        await self._completeIfNotSet(self._tfvc_project)
    ...
    @property
    async def use_lfs(self) -> str:
        await self._completeIfNotSet(self._use_lfs)
        return self._use_lfs.value

    @property
    async def vcs(self) -> str:
        await self._completeIfNotSet(self._vcs)
        return self._vcs.value

    @property
    async def vcs_url(self) -> str:
