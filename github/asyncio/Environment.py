...
class Environment(CompletableGithubObject):
    ...
    async def create_secret(self, secret_name: str, unencrypted_value: str) -> Secret.Secret:
        ...
        url = f"{await self.url}/secrets/{quoted_secret_name}"
    ...
    async def get_secrets(self) -> PaginatedList[Secret.Secret]:
        ...
        return PaginatedList(
            Secret.Secret,
            self._requester,
            f"{await self.url}/secrets",
            None,
            attributesTransformer=PaginatedList.override_attributes({"secrets_url": f"{await self.url}/secrets"}),
            list_item="secrets",
        )
    ...
    async def create_variable(self, variable_name: str, value: str) -> Variable.Variable:
        ...
        await self._requester.requestJsonAndCheck("POST", f"{await self.url}/variables", input=post_parameters)

        quoted_variable_name = urllib.parse.quote(variable_name, safe="")
        url = f"{await self.url}/variables/{quoted_variable_name}"
    ...
    async def get_variables(self) -> PaginatedList[Variable.Variable]:
        ...
        return PaginatedList(
            Variable.Variable,
            self._requester,
            f"{await self.url}/variables",
            None,
            attributesTransformer=PaginatedList.override_attributes({"variables_url": f"{await self.url}/variables"}),
            list_item="variables",
        )
    ...
    async def get_variable(self, variable_name: str) -> Variable.Variable:
        ...
        url = f"{await self.url}/variables/{variable_name}"
    ...
    async def delete_secret(self, secret_name: str) -> bool:
        ...
        """
        assert isinstance(secret_name, str), secret_name
        status, headers, data = await self._requester.requestJson("DELETE", f"{await self.url}/secrets/{secret_name}")
        return status == 204
    ...
    async def delete_variable(self, variable_name: str) -> bool:
        ...
        status, headers, data = await self._requester.requestJson(
            "DELETE", f"{await self.url}/variables/{variable_name}"
        )
        return status == 204
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "protection_rules" in attributes:  # pragma no branch
            self._protection_rules = self._makeListOfClassesAttribute(
                EnvironmentProtectionRule.EnvironmentProtectionRule,
