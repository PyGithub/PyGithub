...
from . import NamedUser, Reaction, RepositoryDiscussion
from .DiscussionCommentBase import DiscussionCommentBase
...
class RepositoryDiscussionComment(GraphQlObject, DiscussionCommentBase):
    ...
    async def edit(self, body: str, output_schema: str = "id") -> RepositoryDiscussionComment:
        if not output_schema.startswith("\n"):
            output_schema = f" {output_schema} "
        return await self._requester.graphql_named_mutation_class(
            "updateDiscussionComment",
            {"commentId": await self.node_id, "body": body},
            f"comment {{{output_schema}}}",
            "comment",
            RepositoryDiscussionComment,
        )

    async def delete(self, output_schema: str = "id") -> RepositoryDiscussionComment:
        if not output_schema.startswith("\n"):
            output_schema = f" {output_schema} "
        return await self._requester.graphql_named_mutation_class(
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "discussion" in attributes:  # pragma no branch
            ...
            self._discussion = self._makeClassAttribute(
                RepositoryDiscussion.RepositoryDiscussion, attributes["discussion"]
            )
        if "editor" in attributes:  # pragma no branch
            # NamedUser is a REST API object
            self._editor = self._makeClassAttribute(NamedUser.NamedUser, as_rest_api_attributes(attributes["editor"]))
