...
class Hook(CompletableGithubObject):
    ...
    def _initAttributes(self) -> None:
        ...
        self._last_response: Attribute[HookResponse.HookResponse] = NotSet
    ...
    async def active(self) -> bool:
        await self._completeIfNotSet(self._active)
        return self._active.value

    @property
    async def config(self) -> dict:
        await self._completeIfNotSet(self._config)
    ...
    @property
    async def deliveries_url(self) -> str:
        await self._completeIfNotSet(self._deliveries_url)
        return self._deliveries_url.value

    @property
    async def events(self) -> list[str]:
        await self._completeIfNotSet(self._events)
    ...
    @property
    async def last_response(self) -> HookResponse.HookResponse:
        await self._completeIfNotSet(self._last_response)
    ...
    @property
    async def ping_url(self) -> str:
        await self._completeIfNotSet(self._ping_url)
        return self._ping_url.value

    @property
    async def test_url(self) -> str:
        await self._completeIfNotSet(self._test_url)
    ...
    async def test(self) -> None:
        ...
        """
        headers, data = await self._requester.requestJsonAndCheck("POST", f"{await self.url}/tests")

    async def ping(self) -> None:
        """
        :calls: `POST /repos/{owner}/{repo}/hooks/{hook_id}/pings <https://docs.github.com/en/rest/reference/repos#webhooks>`_
        """
        headers, data = await self._requester.requestJsonAndCheck("POST", f"{await self.url}/pings")
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "last_response" in attributes:  # pragma no branch
            self._last_response = self._makeClassAttribute(HookResponse.HookResponse, attributes["last_response"])
