...
class WorkflowRun(CompletableGithubObject):
    ...
    @property
    async def artifacts_url(self) -> str:
        await self._completeIfNotSet(self._artifacts_url)
        return self._artifacts_url.value

    @property
    async def cancel_url(self) -> str:
        await self._completeIfNotSet(self._cancel_url)
        return self._cancel_url.value

    @property
    async def check_suite_id(self) -> int:
        await self._completeIfNotSet(self._check_suite_id)
        return self._check_suite_id.value

    @property
    async def check_suite_node_id(self) -> str:
        await self._completeIfNotSet(self._check_suite_node_id)
        return self._check_suite_node_id.value

    @property
    async def check_suite_url(self) -> str:
        await self._completeIfNotSet(self._check_suite_url)
    ...
    async def display_title(self) -> str:
        await self._completeIfNotSet(self._display_title)
        return self._display_title.value

    @property
    async def event(self) -> str:
        await self._completeIfNotSet(self._event)
    ...
    @property
    async def head_repository_id(self) -> int:
        await self._completeIfNotSet(self._head_repository_id)
    ...
    @property
    async def jobs_url(self) -> str:
        await self._completeIfNotSet(self._jobs_url)
        return self._jobs_url.value

    @property
    async def logs_url(self) -> str:
        await self._completeIfNotSet(self._logs_url)
    ...
    @property
    async def previous_attempt_url(self) -> str:
        await self._completeIfNotSet(self._previous_attempt_url)
    ...
    @property
    async def referenced_workflows(self) -> list[dict[str, Any]]:
        await self._completeIfNotSet(self._referenced_workflows)
    ...
    @property
    async def repository_id(self) -> int:
        await self._completeIfNotSet(self._repository_id)
        return self._repository_id.value

    @property
    async def rerun_url(self) -> str:
        await self._completeIfNotSet(self._rerun_url)
    ...
    @property
    async def run_number(self) -> int:
        await self._completeIfNotSet(self._run_number)
        return self._run_number.value

    @property
    async def run_started_at(self) -> datetime:
        await self._completeIfNotSet(self._run_started_at)
    ...
    @property
    async def triggering_actor(self) -> NamedUser.NamedUser:
        await self._completeIfNotSet(self._triggering_actor)
    ...
    @property
    async def workflow_id(self) -> int:
        await self._completeIfNotSet(self._workflow_id)
    ...
    async def get_attempt(self, attempt_number: int) -> WorkflowRun:
        """
        ...
        url = f"{await self.url}/attempts/{attempt_number}"
    ...
    async def cancel(self) -> bool:
        ...
        """
        status, _, _ = await self._requester.requestJson("POST", await self.cancel_url)
        return status == 202

    async def rerun(self) -> bool:
        """
        :calls: `POST /repos/{owner}/{repo}/actions/runs/{run_id}/rerun <https://docs.github.com/en/rest/reference/actions#workflow-runs>`_
        """
        status, _, _ = await self._requester.requestJson("POST", await self.rerun_url)
        return status == 201

    async def rerun_failed_jobs(self) -> bool:
        """
        ...
        """
        status, _, _ = await self._requester.requestJson("POST", f"{await self.url}/rerun-failed-jobs")
        return status == 201

    async def timing(self) -> TimingData:
        """
        :calls: `GET /repos/{owner}/{repo}/actions/runs/{run_id}/timing <https://docs.github.com/en/rest/reference/actions#workflow-runs>`_
        """
        headers, data = await self._requester.requestJsonAndCheck("GET", f"{await self.url}/timing")
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        if "actor" in attributes:  # pragma no branch
            self._actor = self._makeClassAttribute(NamedUser.NamedUser, attributes["actor"])
        ...
        if "head_repository" in attributes:  # pragma no branch
            self._head_repository = self._makeClassAttribute(Repository.Repository, attributes["head_repository"])
        ...
        if "triggering_actor" in attributes:  # pragma no branch
            self._triggering_actor = self._makeClassAttribute(NamedUser.NamedUser, attributes["triggering_actor"])
