...
class NamedEnterpriseUser(CompletableGithubObject):
    ...
    @property
    async def enterprise_server_primary_emails(self) -> list:
        await self._completeIfNotSet(self._enterprise_server_primary_emails)
        return self._enterprise_server_primary_emails.value

    @property
    async def enterprise_server_user(self) -> bool:
        await self._completeIfNotSet(self._enterprise_server_user)
        return self._enterprise_server_user.value

    @property
    async def enterprise_server_user_ids(self) -> list:
        await self._completeIfNotSet(self._enterprise_server_user_ids)
        return self._enterprise_server_user_ids.value

    @property
    async def github_com_enterprise_roles(self) -> list:
        await self._completeIfNotSet(self._github_com_enterprise_roles)
        return self._github_com_enterprise_roles.value

    @property
    async def github_com_login(self) -> str:
        await self._completeIfNotSet(self._github_com_login)
        return self._github_com_login.value

    @property
    async def github_com_member_roles(self) -> list:
        await self._completeIfNotSet(self._github_com_member_roles)
        return self._github_com_member_roles.value

    @property
    async def github_com_name(self) -> str:
        await self._completeIfNotSet(self._github_com_name)
        return self._github_com_name.value

    @property
    async def github_com_orgs_with_pending_invites(self) -> list:
        await self._completeIfNotSet(self._github_com_orgs_with_pending_invites)
        return self._github_com_orgs_with_pending_invites.value

    @property
    async def github_com_profile(self) -> str:
        await self._completeIfNotSet(self._github_com_profile)
        return self._github_com_profile.value

    @property
    async def github_com_saml_name_id(self) -> str:
        await self._completeIfNotSet(self._github_com_saml_name_id)
        return self._github_com_saml_name_id.value

    @property
    async def github_com_two_factor_auth(self) -> bool:
        await self._completeIfNotSet(self._github_com_two_factor_auth)
        return self._github_com_two_factor_auth.value

    @property
    async def github_com_user(self) -> bool:
        await self._completeIfNotSet(self._github_com_user)
        return self._github_com_user.value

    @property
    async def github_com_verified_domain_emails(self) -> list:
        await self._completeIfNotSet(self._github_com_verified_domain_emails)
        return self._github_com_verified_domain_emails.value

    @property
    async def license_type(self) -> str:
        await self._completeIfNotSet(self._license_type)
        return self._license_type.value

    @property
    async def total_user_accounts(self) -> int:
        await self._completeIfNotSet(self._total_user_accounts)
        return self._total_user_accounts.value

    @property
    async def visual_studio_license_status(self) -> str:
        await self._completeIfNotSet(self._visual_studio_license_status)
        return self._visual_studio_license_status.value

    @property
    async def visual_studio_subscription_email(self) -> str:
        await self._completeIfNotSet(self._visual_studio_subscription_email)
        return self._visual_studio_subscription_email.value

    @property
    async def visual_studio_subscription_user(self) -> bool:
        await self._completeIfNotSet(self._visual_studio_subscription_user)
