...
from .GithubObject import CompletableGithubObject, GithubObject, NotSet, Opt, is_defined, is_undefined
from .HookDelivery import HookDelivery, HookDeliverySummary
from .HookDescription import HookDescription
from .PaginatedList import PaginatedList
from .RateLimitOverview import RateLimitOverview
...
class Github:
    ...
    def __init__(
        ...
        """
        :param login_or_token: string deprecated, use auth=Auth.Login(...) or auth=Auth.Token(...) instead
        :param password: string deprecated, use auth=Auth.Login(...) instead
        :param jwt: string deprecated, use auth=Auth.AppAuth(...) or auth=Auth.AppAuthToken(...) instead
        :param app_auth: github.AppAuthentication deprecated, use auth=Auth.AppInstallationAuth(...) instead
        ...
        assert auth is None or isinstance(auth, (Auth.Auth, github.Auth.Auth)), auth
        ...
        if password is not None:
            ...
            warnings.warn(
                ...
                "auth=Auth.AppAuth(...) or "
                "auth=Auth.AppAuthToken(...) instead",
                category=DeprecationWarning,
                stacklevel=2,
            )
    ...
    @property
    async def rate_limiting(self) -> tuple[int, int]:
        """
    ...
    @property
    async def rate_limiting_resettime(self) -> int:
        """
    ...
    async def get_rate_limit(self) -> RateLimitOverview:
        """
        ...
        """
        headers, data = await self.__requester.requestJsonAndCheck("GET", "/rate_limit")
    ...
    def get_user_by_id(self, user_id: int) -> NamedUser.NamedUser:
        ...
        :param user_id: int
        :rtype: :class:`NamedUser.NamedUser`
        """
    ...
    def get_users(self, since: Opt[int] = NotSet) -> PaginatedList[NamedUser.NamedUser]:
        ...
        """
        assert is_undefined(since) or isinstance(since, int), since
        url_parameters = dict()
        if is_defined(since):
    ...
    def get_enterprise(self, enterprise: str) -> Enterprise.Enterprise:
        """
    ...
    def get_repo(self, full_name_or_id: int | str, lazy: Opt[bool] = NotSet) -> Repository.Repository:
        ...
        return Repository.Repository(self.__requester.withLazy(lazy), url=url)
    ...
    async def get_global_advisory(self, ghsa_id: str) -> GlobalAdvisory.GlobalAdvisory:
        ...
        :rtype: :class:`GlobalAdvisory.GlobalAdvisory`
        """
        assert isinstance(ghsa_id, str), ghsa_id
        ghsa_id = urllib.parse.quote(ghsa_id)
        headers, data = await self.__requester.requestJsonAndCheck("GET", f"/advisories/{ghsa_id}")
        return GlobalAdvisory.GlobalAdvisory(self.__requester, headers, data)
    ...
    def get_global_advisories(
        ...
        :rtype: :class:`PaginatedList` of :class:`GlobalAdvisory.GlobalAdvisory`
        """
        assert is_undefined(type) or isinstance(type, str), type
        assert is_undefined(ghsa_id) or isinstance(ghsa_id, str)
        assert is_undefined(cve_id) or isinstance(cve_id, str), cve_id
        assert is_undefined(ecosystem) or isinstance(ecosystem, str), ecosystem
        assert is_undefined(severity) or isinstance(severity, str), severity
        assert is_undefined(cwes) or isinstance(cwes, list) or isinstance(cwes, str), cwes
        assert is_undefined(is_withdrawn) or isinstance(is_withdrawn, bool), is_withdrawn
        assert is_undefined(affects) or isinstance(affects, list) or isinstance(affects, str), affects
        assert is_undefined(published) or isinstance(published, str), published
        assert is_undefined(updated) or isinstance(updated, str), updated
        assert is_undefined(modified) or isinstance(modified, str), modified
        assert is_undefined(before) or isinstance(before, str), before
        assert is_undefined(after) or isinstance(after, str), after
        ...
        if is_defined(type):  # pragma no branch (Should be covered)
            assert type in ("reviewed", "unreviewed", "malware"), type
            url_parameters["type"] = type
        if is_defined(ghsa_id):
    ...
    def search_repositories(
        ...
        **qualifiers: Any,
        ...
        :rtype: :class:`PaginatedList` of :class:`Repository.RepositorySearchResult`
        ...
        url_parameters = dict()
        if is_defined(sort):  # pragma no branch (Should be covered)
            assert sort in ("stars", "forks", "updated"), sort
            url_parameters["sort"] = sort
        if is_defined(order):  # pragma no branch (Should be covered)
    ...
    def search_users(
        ...
        **qualifiers: Any,
        ...
        :param order: string ('asc', 'desc')
        :param qualifiers: keyword dict query qualifiers
        :rtype: :class:`PaginatedList` of :class:`NamedUser.NamedUserSearchResult`
        ...
        return PaginatedList(
            NamedUser.NamedUserSearchResult,
            self.__requester,
            "/search/users",
            url_parameters,
        )
    ...
    def search_issues(
        ...
        **qualifiers: Any,
        ...
        :param order: string ('asc', 'desc')
        :param qualifiers: keyword dict query qualifiers
        :rtype: :class:`PaginatedList` of :class:`Issue.IssueSearchResult`
        ...
        return PaginatedList(Issue.IssueSearchResult, self.__requester, "/search/issues", url_parameters)
    ...
    def search_code(
        ...
        **qualifiers: Any,
        ...
        :param qualifiers: keyword dict query qualifiers
        :rtype: :class:`PaginatedList` of :class:`ContentFile.ContentFileSearchResult`
        ...
        return PaginatedList(
            ContentFile.ContentFileSearchResult,
            self.__requester,
            "/search/code",
            url_parameters,
            headers=headers,
    ...
    def search_commits(
        ...
        **qualifiers: Any,
        ...
        :param order: string ('asc', 'desc')
        :param qualifiers: keyword dict query qualifiers
        :rtype: :class:`PaginatedList` of :class:`Commit.CommitSearchResult`
    ...
    def search_topics(self, query: str, **qualifiers: Any) -> PaginatedList[Topic.Topic]:
        ...
        :param query: string
        :param qualifiers: keyword dict query qualifiers
        :rtype: :class:`PaginatedList` of :class:`Topic.Topic`
    ...
    async def render_markdown(self, text: str, context: Opt[Repository.Repository] = NotSet) -> str:
        ...
        :param text: string
        :param context: :class:`Repository.Repository`
        :rtype: string
        """
        assert isinstance(text, str), text
        assert is_undefined(context) or isinstance(
            context, (Repository.Repository, github.Repository.Repository)
        ), context
        post_parameters = {"text": text}
        if is_defined(context):
            post_parameters["mode"] = "gfm"
            post_parameters["context"] = await context._identity
        status, headers, data = await self.__requester.requestJson("POST", "/markdown", input=post_parameters)
        return data

    async def get_hook(self, name: str) -> HookDescription:
        """
        :calls: `GET /hooks/{name} <https://docs.github.com/en/rest/reference/repos#webhooks>`_
        """
        assert isinstance(name, str), name
        name = urllib.parse.quote(name, safe="")
        headers, attributes = await self.__requester.requestJsonAndCheck("GET", f"/hooks/{name}")
        return HookDescription(self.__requester, headers, attributes)

    async def get_hooks(self) -> list[HookDescription]:
        """
        :calls: `GET /hooks <https://docs.github.com/en/rest/reference/repos#webhooks>`_
        :rtype: list of :class:`HookDescription`
        """
        headers, data = await self.__requester.requestJsonAndCheck("GET", "/hooks")
        return [HookDescription(self.__requester, headers, attributes) for attributes in data]

    async def get_hook_delivery(self, hook_id: int, delivery_id: int) -> HookDelivery:
        """
        ...
        headers, attributes = await self.__requester.requestJsonAndCheck(
            "GET", f"/hooks/{hook_id}/deliveries/{delivery_id}"
        )
        return HookDelivery(self.__requester, headers, attributes)

    async def get_hook_deliveries(self, hook_id: int) -> list[HookDeliverySummary]:
        """
        ...
        """
        assert isinstance(hook_id, int), hook_id
        headers, data = await self.__requester.requestJsonAndCheck("GET", f"/hooks/{hook_id}/deliveries")
        return [HookDeliverySummary(self.__requester, headers, attributes) for attributes in data]

    async def get_gitignore_templates(self) -> list[str]:
        """
    ...
    async def get_gitignore_template(self, name: str) -> GitignoreTemplate.GitignoreTemplate:
        ...
        headers, attributes = await self.__requester.requestJsonAndCheck("GET", f"/gitignore/templates/{name}")
        return GitignoreTemplate.GitignoreTemplate(self.__requester, headers, attributes)
    ...
    async def get_emojis(self) -> dict[str, str]:
        ...
        """
        headers, attributes = await self.__requester.requestJsonAndCheck("GET", "/emojis")
        return attributes
    ...
    async def dump(self, obj: GithubObject, file: BinaryIO, protocol: int = 0) -> None:
        """
    ...
    async def get_app(self, slug: Opt[str] = NotSet) -> GithubApp.GithubApp:
        ...
        if is_undefined(slug):
            ...
            warnings.warn(
                ...
                "github.GithubIntegration(auth=Auth.AppAuth(...)).get_app() instead",
                category=DeprecationWarning,
                stacklevel=2,
            )
