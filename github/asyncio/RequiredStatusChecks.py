...
class RequiredStatusChecks(CompletableGithubObject):
    ...
    async def checks(self) -> list[Check]:
        await self._completeIfNotSet(self._checks)
        return self._checks.value

    @property
    async def contexts(self) -> list[str]:
        await self._completeIfNotSet(self._contexts)
        return self._contexts.value

    @property
    async def contexts_url(self) -> str:
        await self._completeIfNotSet(self._contexts_url)
        return self._contexts_url.value

    @property
    async def enforcement_level(self) -> str:
        await self._completeIfNotSet(self._enforcement_level)
        return self._enforcement_level.value

    @property
    async def strict(self) -> bool:
        await self._completeIfNotSet(self._strict)
