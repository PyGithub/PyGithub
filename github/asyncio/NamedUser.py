...
class NamedUser(GithubObject.CompletableGithubObject):
    ...
    def __eq__(self, other: Any) -> bool:
        return (
            ...
            and self._login.value == other._login.value
            and self._id.value == other._id.value
        )

    def __hash__(self) -> int:
        return hash((self._id.value, self._login.value))
    ...
    @property
    async def collaborators(self) -> int | None:
    ...
    @property
    async def contributions(self) -> int:
        await self._completeIfNotSet(self._contributions)
    ...
    @property
    async def invitation_teams_url(self) -> str:
        await self._completeIfNotSet(self._invitation_teams_url)
        return self._invitation_teams_url.value

    @property
    async def inviter(self) -> NamedUser:
    ...
    @property
    async def owned_private_repos(self) -> int | None:
    ...
    @property
    async def private_gists(self) -> int | None:
    ...
    @property
    async def suspended_at(self) -> datetime | None:
        await self._completeIfNotSet(self._suspended_at)
        return self._suspended_at.value

    @property
    async def team_count(self) -> int:
        await self._completeIfNotSet(self._team_count)
    ...
    async def get_followers(self) -> PaginatedList[NamedUser]:
        """
        :calls: `GET /users/{username}/followers <https://docs.github.com/en/rest/reference/users#followers>`_
        """
        return PaginatedList(NamedUser, self._requester, f"{await self.url}/followers", None)

    async def get_following(self) -> PaginatedList[NamedUser]:
        """
    ...
    async def get_public_received_events(self) -> PaginatedList[Event]:
        ...
        return PaginatedList(
            Event.Event,
            self._requester,
            f"{await self.url}/received_events/public",
            None,
        )
    ...
    async def get_subscriptions(self) -> PaginatedList[Repository]:
        ...
        return PaginatedList(
            Repository.Repository,
            self._requester,
            f"{await self.url}/subscriptions",
            None,
        )
    ...
    async def get_watched(self) -> PaginatedList[Repository]:
        ...
        """
        return PaginatedList(Repository.Repository, self._requester, f"{await self.url}/watched", None)
    ...
    async def has_in_following(self, following: NamedUser) -> bool:
        ...
        status, headers, data = await self._requester.requestJson(
            "GET", f"{await self.url}/following/{(await following._identity)}"
        )
        return status == 204
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if "inviter" in attributes:  # pragma no branch
            self._inviter = self._makeClassAttribute(NamedUser, attributes["inviter"])
...
class OrganizationInvitation(NamedUser):
    ...
    async def cancel(self) -> bool:
        ...
        :rtype: bool
        """
        status, headers, data = await self._requester.requestJson("DELETE", await self.url)
        return status == 204
