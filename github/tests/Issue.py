...
class Issue(Framework.TestCase):
    ...
    def testAttributes(self):
        ...
        self.assertEqual(self.issue.closed_at,
                         datetime.datetime(2012, 5, 26, 14, 59, 33))
        self.assertEqual(self.issue.closed_by.login, "jacquev6")
        self.assertEqual(self.issue.comments, 0)
        self.assertEqual(self.issue.created_at,
                         datetime.datetime(2012, 5, 19, 10, 38, 23))
        ...
        self.assertEqual(self.issue.updated_at,
                         datetime.datetime(2012, 5, 26, 14, 59, 33))
        self.assertEqual(
        ...
        self.assertEqual(self.issue.__repr__(),
    ...
    def testEditWithAllParameters(self):
        ...
        self.issue.edit("Title edited by PyGithub",
                        "Body edited by PyGithub", user, "open", milestone, ["Bug"])
        self.assertEqual(self.issue.assignee.login, "jacquev6")
        self.assertEqual(self.issue.body, "Body edited by PyGithub")
    ...
    def testGetComments(self):
        self.assertListKeyEqual(self.issue.get_comments(), lambda c: c.user.login, [
                                "jacquev6", "roskakori"])

    def testGetCommentsSince(self):
        self.assertListKeyEqual(self.issue.get_comments(datetime.datetime(
            2012, 5, 26, 13, 59, 33)), lambda c: c.user.login, ["jacquev6", "roskakori"])

    def testGetEvents(self):
        self.assertListKeyEqual(self.issue.get_events(),
                                lambda e: e.id, [15819975, 15820048])
    ...
    def testAddAndRemoveLabels(self):
        ...
        self.assertListKeyEqual(self.issue.get_labels(),
                                lambda l: l.name, ["Project management"])
