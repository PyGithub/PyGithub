...
class BadAttributes(Framework.TestCase):
    ...
    def testBadSimpleAttribute(self):
        ...
        self.assertEqual(user.created_at, datetime.datetime(
            2011, 3, 23, 15, 42, 9))
    ...
    def testBadAttributeTransformation(self):
        ...
        try:
            ...
            if Framework.atLeastPython26:
                self.assertEqual(e.transformation_exception.args,
                                 ("time data 'foobar' does not match format '%Y-%m-%dT%H:%M:%SZ'",))
            else:
                self.assertEqual(e.transformation_exception.args, (
                    'time data did not match format:  data=foobar  fmt=%Y-%m-%dT%H:%M:%SZ',))
        self.assertTrue(raised)
        ...
        for hook in hooks:
            if hook.name == "circleci":
                try:
                    ...
                    raised = True
                    self.assertEqual(e.actual_value, [["commit_comment", "create", "delete", "download", "follow", "fork", "fork_apply", "gist", "gollum",
                                                       "issue_comment", "issues", "member", "public", "pull_request", "pull_request_review_comment", "push", "status", "team_add", "watch"]])
                    self.assertEqual(e.expected_type, [(str, unicode)])
                    self.assertEqual(e.transformation_exception, None)
        self.assertTrue(raised)
