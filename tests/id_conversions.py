...
class TestIDConversions(Framework.TestCase):
    ...
    def test_db_id_and_type_to_node_id(self):
        self.assertEqual(
            db_id_and_type_to_node_id(
                self.__class__.TEST_DB_ID, self.__class__.TEST_GQL_TYPE_NAME
            ),
            self.__class__.TEST_GQL_ID
        )

    def test_node_id_to_db_id_and_type(self):
        self.assertEqual(
            node_id_to_db_id_and_type(self.__class__.TEST_GQL_ID),
            (self.__class__.TEST_DB_ID, self.__class__.TEST_GQL_TYPE_NAME)
        )
