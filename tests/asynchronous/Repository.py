    ...
    def testAsUrlParam(self):
        self.assertEqual(
            object.__getattribute__(self.repo, "_loop").run_until_complete(
                github.asynchronous.Repository.Repository.as_url_param(SyncProxy._deep_unwrap(self.repo))
            ),
            "PyGithub/PyGithub",
        )
        self.assertEqual(
            object.__getattribute__(self.repo, "_loop").run_until_complete(
                github.asynchronous.Repository.Repository.as_url_param(SyncProxy._deep_unwrap(self.repo._identity))
            ),
            "PyGithub/PyGithub",
        )
        ...
        for repo in ["repo", "repo/name/slash"]:
            with self.assertRaises(AssertionError) as raisedexp:
                object.__getattribute__(self.repo, "_loop").run_until_complete(
                    github.asynchronous.Repository.Repository.as_url_param(SyncProxy._deep_unwrap(repo))
                )
