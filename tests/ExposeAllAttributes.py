...
class ExposeAllAttributes(Framework.TestCase):
    def testAllClasses(self):
        ...
        milestone = repository.get_milestone(1)
        ...
        issueComment = issue.get_comment(672573285)
        issueEvent = issue.get_events()[0]
        issuePullRequest = issue.pull_request
        gitignoreTemplate = self.g.get_gitignore_template("Python")
        team = organization.get_team(3997153)
        label = repository.get_label("Bug")
        pullRequest = repository.get_pull(1)
        pullRequestComment = pullRequest.get_review_comment(469006394)
        pullRequestPart = pullRequest.base
        file = pullRequest.get_files()[0]
        commitComment = repository.get_comment(41395834)
        rateLimit = self.g.get_rate_limit()
        hook = repository.get_hooks()[0]
        hookResponse = hook.last_response
        comparison = repository.compare("main", "workflow_dispatch")
        contentFile = repository.get_contents("README.md")
        permissions = repository.permissions
        event = repository.get_events()[0]
        notification = authenticatedUser.get_notification("885161191")
